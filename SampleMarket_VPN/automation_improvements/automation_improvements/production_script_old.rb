...
describe "Test 00: Production basic tests" do
 ...
 it "Test to check if all the mandatory fields for sending a email broadcast are compulsarily filled" do
    ...
    it "Test to check if user is able to update the criteria and check counts" do
      sleep 3
...
#*****************************************************
	...
	@browser.select_list(:id,"rdTime").value.should include("PM")
      ...
      it "Test to check Review broadcast-check details" do
        ...
        body_text.should include("Surveyhead -- 1 email")
...
=end    
	...
	# SangeetaPai- 12 March 2013 - start changes - commented the previous fix and refixed
		...
		if(@browser1.div(:id=>"desktopCover").exists?)
			puts "before click -complete"
			@browser1.div(:id=>"desktopCover").click
			puts "after click -complete"
		end
		sleep 5
...
=end      
	...
	# SangeetaPai- 12 March 2013 - start changes - commented the previous fix and refixed
		...
		if(@browser1.div(:id=>"desktopCover").exists?)			
		puts "before click -fail"
			@browser1.div(:id=>"desktopCover").click
			puts "after click - fail"
		end
		sleep 5
...
=end      
    ...
    it "Test to check if the email broadcast is successfully received" do
      ...
      @browser1.goto('http://sangeeta_pai.mailinator.com')
      #@browser1.text_field(:name,"email").set("sangeeta_pai")
