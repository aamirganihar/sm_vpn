...
class Test_61_Rewards_at_site_level < Test::Unit::TestCase
	  ...
	  def test_02_set_pl_rewards
			...
			begin	
				...
				$ie.goto("http://p.usampadmin.com/add_site.php?site_id=#{$enc_site_id}")
				sleep 3
				...
				if($ie.checkbox(:id, "cbCustomIncentiveGrid").isSet?)
					puts "PASS"
				...
				$myfile.print "<tr><td class=\"td1\">"+$st+"</td>"
				$myfile.print "<td class=\"td2\">"+$test_description+"</td>"
				if($ie.checkbox(:id, "chkFailOverquotaFlag").isSet? && $ie.radio(:id, "rdCashtype").isSet?)
					puts "PASS"
                    $myfile.print "<td class=\"td3\"><font color=\"green\">TEST PASSED</font></td></tr>"
				else
					puts "FAIL"
	  ...
	  def test_03_success_reward
			...
			begin		
					...
					$ff.attach(:url,"http://www.p.surveyhead.com/dashboard.php")
                    ...
                    if($html_array[index] =~ /#{$qg_id.chomp}/)
							...
							puts $html_array[index+8]
							puts "********"
							if (($html_array[index+9] =~ /Earned/) && ($html_array[index+5] =~ /\$0.75/))
                                    puts "PASS"
                                    $myfile.print "<td class=\"td3\"><font color=\"green\">TEST PASSED</font></td></tr>"
                            else
                                    puts "FAIL"
	  ...
	  def test_04_fail_reward
			...
			begin		
                    ...
                    if($html_array[index] =~ /#{$qg_id.chomp}/)
							...
							puts $html_array[index+8]
							puts "********"
							if (($html_array[index+9] =~ /Earned/) && ($html_array[index+5] =~ /\$0.25/))
                                    puts "PASS"
                                    $myfile.print "<td class=\"td3\"><font color=\"green\">TEST PASSED</font></td></tr>"
                            else
                                    puts "FAIL"
