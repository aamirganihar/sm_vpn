...
class Test_Test_39_Add_client < Test::Unit::TestCase
	  ...
	  def test_02_add_client
			...
			begin	
				...
				$ie.select_list(:name, "optAllEmpRep").set($rep)
				...
				sleep 2
				if($ie.contains_text("Client details have been added successfully."))
					puts 'Pass'
	  ...
	  def test_03_search_edit_client
			...
			begin	
				...
				$ie = $obj.Usampadmin_login($admin_email,$admin_passwd)
				sleep 2
				$ie.goto("http://p.usampadmin.com/list_clients.php")
				$ie.text_field(:name, "txtCompany").set($clnt_comp)
				...
				if($ie.contains_text(/updated successfully/) && $clnt_n == "TEST TEMP CLNT COMP")
					puts 'Pass'
					$myfile.print "<td class=\"td3\"><font color=\"green\">TEST PASSED</font></td></tr>"
				else
					puts 'Fail'   
