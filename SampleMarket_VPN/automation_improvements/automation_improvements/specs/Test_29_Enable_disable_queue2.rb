...
class Test_29_Enable_disable_queue2 < Test::Unit::TestCase
	  ...
	  def test_02_enable_q2
			...
			begin	
				...
				if ($ie.checkbox(:id,'chbAllowQ2').isSet?)
						puts "Q2:Enabled"
	  ...
	  def test_03_check_queue2_enabled
			...
			begin	
				...
				sleep 10
				$ff.select_list(:id,"optCountryId").set($country)
				...
				$ff.radio(:name,"rdGender",$gender).set
				...
				$myfile.print "<tr><td class=\"td1\">"+$st+"</td>"
				$myfile.print "<td class=\"td2\">"+$test_description+"</td>"
				if ($ff.url == "http://www.p.surveyhead.com/queue2.php")
						puts "PASS"
						$myfile.print "<td class=\"td3\"><font color=\"green\">TEST PASSED</font></td></tr>"
				else
						puts "FAIL"
