...
class Test_63_Broadcasts < Test::Unit::TestCase
	  ...
	  def test_04_send_reminder
			...
			begin	
				...
				$ie.button(:value,"Get Count").click
				sleep 10 
				$ie.radio(:name, "radiobuttonGroup1", "rdReminder").click
				sleep 3
				$ie.text_field(:id, "txtNoEmailsSenduSampPanel").set("1")
				$ie.button(:value,"Next").click
				$ie.select_list(:name, "optTemplate").set("Client Services - Template A")
	  ...
	  def test_05_complete_survey
			...
			begin	
				...
				$myfile.print "<tr><td class=\"td1\">"+$st+"</td>"
				$myfile.print "<td class=\"td2\">"+$test_description+"</td>"
				if($ff.contains_text(/Congratulations, you've just completed the survey/))
					puts "PASS"
					$myfile.print "<td class=\"td3\"><font color=\"green\">TEST PASSED</font></td></tr>"
				else
					puts"FAIL"
	  ...
	  def test_07_schedule_bcast
			...
			begin	
				...
				$ie.text_field(:id, "tagsSubject").set($b_subj)
				$ie.button(:value,"Next").click
				$ie.radio(:name, "rdSchedule", "scheduleOn").click
				...
				$ie.select_list(:name, "optTimeHour").set($hour)
				$ie.select_list(:name, "optTimeMin").set(/#{$min}/)
				if ($flg == 0)
