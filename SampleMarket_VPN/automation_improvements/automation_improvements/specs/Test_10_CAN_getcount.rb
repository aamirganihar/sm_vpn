...
class Test_10_CAN_getcount < Test::Unit::TestCase
		...
		def test_02_CAN_getcount
				...
				begin
						...
						if (ff.contains_text("CAN GETCOUNT SURVEY"))
                        ...
                        $ie.select_list(:id, "optProvince_1").set($state)
                        sleep 4
                        $ie.select_list(:id, "optCanadaCity_1").set($city)
                        $ie.button(:value,"Save Group").click
                        sleep 2
                        ff.link(:text,"Dashboard").click
                        sleep 5
						...
						$myfile.print "<td class=\"td2\">"+$test_description+"</td>"
						sleep 25
						if (ff.contains_text("CANADA GETCOUNT SURVEY"))
                                $myfile.print "<td class=\"td3\"><font color=\"red\">TEST FAILED</font></td></tr>"
		...
		def test_04_cangetcnt_mempub
				...
				begin	
					...
					$ff.goto('http://sm.p.surveyhead.com/index.php?mode=logout')
					$ff.goto("p.surveyhead.com/recaptcha_automation_proxy.php?mode=normal")
					sleep 2
					$ff.close
