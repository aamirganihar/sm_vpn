...
class Test_52_Profile_criteria_cases < Test::Unit::TestCase
	...
	def test_02_prof_create_add_questions
			...
			begin	
				...
				$ie.select_list(:id, "existing_profile").set(/#{$prf_name}/)
				sleep 5
				$ie.select_list(:id, "existing_question").set(/#{$exs_quest}/)
				sleep 2
				$ie.button(:value,"Add").click
				if($ie.contains_text(/Question added successfully/) && $ie.contains_text(/#{$exs_quest}/))
					puts "PASS"
        ...
        $ie.button(:name, "Submit").click
				...
				$ie.select_list(:name, "optProfileId").set(/#{$prf_name}/)
				$ie.select_list(:id, "optProfileQuestionId").set(/#{$crit_quest}/)
				$ie.select_list(:id, "optAnswerId[]").set("MERC")
        sleep 2
				$ie.button(:value,"Add").click
        sleep 5
				if($ie.contains_text(/#{$crit_quest}/))
					puts "PASS"
					$myfile.print "<td class=\"td3\"><font color=\"green\">TEST PASSED</font></td></tr>"
				else
				    puts "FAIL"
				...
				sleep 3
				$ie.select_list(:id, "setActionType").set("SKIP TO")
				...
				sleep 3
				if($ie.contains_text(/Question updated successfully/))
					puts "PASS"
					$myfile.print "<td class=\"td3\"><font color=\"green\">TEST PASSED</font></td></tr>"
				else
				    puts "FAIL"
	...
	def test_04_checking_on_pl
			...
			begin	
				...
				$myfile.print "<tr><td class=\"td1\">"+$st+"</td>"
				$myfile.print "<td class=\"td2\">"+$test_description+"</td>"
				if($ff.contains_text(/TEST OPENENDED/))
						puts "PASS"
						$myfile.print "<td class=\"td3\"><font color=\"green\">TEST PASSED</font></td></tr>"
				else
						puts "FAIL"
	...
	def test_05_act_inact
			...
			begin	
				...
				$ie.radio(:name, "rdbProfileStatus", "INACTIVE").click
				...
				$ie.button(:value,"Back").click
				$ie.radio(:name, "rdbProfileStatus", "ACTIVE").click
				$ie.button(:value,"Update & Next").click
				$st = '10'
				...
				$myfile.print "<tr><td class=\"td1\">"+$st+"</td>"
				$myfile.print "<td class=\"td2\">"+$test_description+"</td>"
				if($ie.contains_text(/TEST DROPDOWN/))
					puts "FAIL"
					$myfile.print "<td class=\"td3\"><font color=\"red\">TEST FAILED</font></td></tr>"
				else
					puts "PASS"
				...
				$myfile.print "<td class=\"td2\">"+$test_description+"</td>"
				$ff.button(:value,"Next").click
				if($ff.contains_text(/TEST DROPDOWN/))
					puts "FAIL"
					$myfile.print "<td class=\"td3\"><font color=\"red\">TEST FAILED</font></td></tr>"
				else
					puts "PASS"
				...
				$ie.goto("http://p.usampadmin.com/list_questions.php?PrId=#{$enc_prof_2_id}")
				$ie.select_list(:id, "optStatus").set("Hidden Questions")
				sleep 2
				$ie.checkbox(:index,"1").set
				$ie.button(:value,"Make Selected Questions Active").click
				sleep 2
				$ie.select_list(:id, "optStatus").set("Active Questions")
				$st = '13'
