...
class Test_XX_Profiles_rewards_inactivate < Test::Unit::TestCase
	  ...
	  def test_02_inactivate_profiles
			...
			begin	
				...
				sleep 3
				#PROFILE 1
				$ie.goto("http://p.usampadmin.com/add_profile.php?intProfileId=#{$prf_1}")
				sleep 2
				...
				$ie.goto("http://p.usampadmin.com/add_profile.php?intProfileId=#{$prf_1}")
				sleep 2
				if ($ie.radio(:value,"INACTIVE").isSet?)
					puts "PASS"
				...
				$ie.goto("http://p.usampadmin.com/add_profile.php?intProfileId=#{$prf_2}")
				sleep 2
				$ie.radio(:name, "rdbProfileStatus", "INACTIVE").click
				$ie.button(:value,"Update & Next").click
				sleep 2
	  ...
	  def test_03_inactivate_rewards
			...
			begin	
				...
				$ie = $obj.Usampadmin_login($admin_email,$admin_passwd)
				sleep 3
				$ie.goto("http://p.usampadmin.com/list_reward_types.php")
				sleep 1
				...
				$ie.goto("http://p.usampadmin.com/list_reward_types.php")
				sleep 1
				$ie.goto("http://p.usampadmin.com/add_reward_type.php?rid=#{$gift_rew_id}")
				sleep 8
				$ie.radio(:value, "INACTIVE").set
				$ie.button(:value,"Update").click
				sleep 2
