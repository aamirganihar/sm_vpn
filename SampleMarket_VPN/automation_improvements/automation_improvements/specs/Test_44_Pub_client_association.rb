...
class Test_44_Pub_client_association < Test::Unit::TestCase
	  ...
	  def test_02_make_pub_level_setting
			...
			begin	
				...
				$ie.goto("http://p.usampadmin.com/add_publisher_settings.php?hdMode=settings&publisher_id=#{$enc_pub_id}")
				sleep 2
				...
				if($flg == 1)
					...
					$ie.checkbox(:id, "chbAssociatePub").set
					$ie.select_list(:id, "optAllUnassocClient").set($client_name)
				...
				sleep 2
				if ($ie.contains_text(/Publisher Settings have been updated successfully/) && $fl == 1)
					puts "Pass"
	  ...
	  def test_03_check_client_panel_builder_setting
			...
			begin	
				...
				$ie.goto("http://p.usampadmin.com/selfserve_panel.php?client_id=#{$enc_client_id}")
				sleep 2
				$st = '2'
				$test_description = "Check Publisher exists on Client panel builder after association "
				$myfile.print "<tr><td class=\"td1\">"+$st+"</td>"
				$myfile.print "<td class=\"td2\">"+$test_description+"</td>"
				if ($ie.contains_text(/#{$pub_id}/))
					puts "Pass"
					$myfile.print "<td class=\"td3\"><font color=\"green\">TEST PASSED</font></td></tr>"
				else
					puts "Fail"
	  ...
	  def test_04_remove_pub_client_association
			...
			begin	
				...
				$myfile.print "<tr><td class=\"td1\">"+$st+"</td>"
				$myfile.print "<td class=\"td2\">"+$test_description+"</td>"
				if ($ie.checkbox(:id, "chbAssociatePub").isSet?)
					puts "Fail"
					$myfile.print "<td class=\"td3\"><font color=\"red\">TEST FAILED</font></td></tr>"
				else
					puts "Pass"
