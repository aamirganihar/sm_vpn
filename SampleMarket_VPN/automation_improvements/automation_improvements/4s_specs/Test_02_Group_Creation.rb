...
#      while @browser.div(:id=>"fancybox-loading").visible? do
  ...
  #end
  #sleep 4
  Watir::Wait.until { @browser.text.include? 'Area Code' }#explicit wait:default timeout :30sec:HRISHI
  ...
  it "Test to check if the test link options are displayed in case data is entered correctly" do
      ...
      Watir::Wait.until { @browser.text.include? 'Add Supplier' }#explicit wait:default timeout :30sec:HRISHI
