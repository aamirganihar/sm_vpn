...
#    @browser.link(:text,/#{project_name['project'][1]}/).click
    ...
    @browser.text_field(:name,"membersTokens").set("23236901")
...
@browser.link(:id,"groupChannelRewardDisplay0").click
    ...
    it "To click on the email invitation link for a survey" do
	  ...
	  #@browser1.driver.manage.timeouts.implicit_wait = 1000
      #sleep 150
      #@browser1.text_field(:name,"email").set("test22.des0091")

   #sleep 240
Watir::Wait.until(300) {@browser1.link(:text=>/less than a minute/).exists?}#explicit wait ..wait for the  sent mail to be recieved, timeout :300 secs
sleep 2
@browser1.link(:text,/Test automation broadcast/).click
     ...
     sleep 15
      ...
      if(html_array[index] =~ /p\.ipoll/)
      #if(html_array[index] =~ /\.p\.surveyhead/)
      #if(html_array[index] =~ //)
        #puts "***"
        ...
        puts @code
        @code1 = @code.slice(128..271)
      puts @code1
      #@code1 = @code1.gsub(/amp;/, "")
      #puts @code1
      @browser1.goto("#{@code1}")
	  #@browser1.driver.manage.timeouts.implicit_wait = 30
      Watir::Wait.until(100){ @browser1.text.include? 'Test question' }#explicit wait:default timeout :30sec
      @browser1.goto("http://p.sm1mr.com/ssred.php?S=1&ID=")
	 sleep 15
      @browser1.text.should include('Congratulations on completing your first survey!') or ('Thank you for your participation. This survey transaction will be reviewed within 4-6 weeks.')
     @browser1.close
     end
