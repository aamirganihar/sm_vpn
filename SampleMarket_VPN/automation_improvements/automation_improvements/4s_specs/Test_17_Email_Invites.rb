...
#    File.open("InputRepository/groupname_Demo.yml","w"){|file| YAML.dump(group_name,file)}
    ...
    it "Test to check if user is able to save settings on clicking next" do
      ...
      sleep 3
      @browser.text_field(:id,"txtEmail1").set("test22.des00_FL@mailinator.com")
	...
	break
      ...
      #@code1 = @code.slice(108..111)
      #puts @code
      @code1 = @code.slice(168..172)
      puts "number"
      puts @code1
      @code1 = @code1.gsub(/,/, "")
      @number = @code1.to_i
...
puts @code
	...
	#end
	#sleep 5
	Watir::Wait.until { @browser.text.include? 'Zip Code' }#explicit wait:default timeout :30sec:HRISHI
   ...
   Watir::Wait.until { @browser.text.include? 'Age' }#explicit wait:default timeout :30sec:HRISHI
	...
	@browser.select_list(:id,"rdTime").value.should include("PM")
      ...
      it "Test to check Review broadcast-check details" do
        ...
        body_text.should include("iPoll -- 5 emails")
...
=end    
    it "Test to check if the email broadcast is successfully received" do
      ...
      puts "waiting for the email to be received"
      #sleep 60
      #@browser1.goto("http://www.mailinator.com/maildir.jsp?email=test_automation")
      @browser1.goto('http://mailinator.com/inbox.jsp?to=test22.des00_fl')
      
      Watir::Wait.until(240) {@browser1.link(:text=>/Test automation broadcast/).exists?}#explicit wait ..wait for the  sent mail to be recieved, timeout :240 secs
