...
describe "Test 31: Fielding limits ASAP" do
  ...
  before(:all) do
    ...
    File.open("InputRepository/projectname_asap.yml","w"){|file| YAML.dump(project_name,file)}
...
#    @browser.link(:text,/#{project_name['project'][1]}/).click
    ...
    #group_name = {}
    #group_name['group_name'] = grp_name
    
    #File.open("InputRepository/Group_Ids_limits.yml","w"){|file| YAML.dump(group_name,file)}
    ...
    @browser.link(:text,/Test Automation Group/).click
     ...
     File.open("InputRepository/Group_Ids_limits_asap.yml","w"){|file| YAML.dump(group_id,file)}
  ...
  it "To check if the state of the project changes to closed after setting it to autoclose after 2 clicks" do
    sleep 90
    @browser.refresh
    #puts @browser.text
