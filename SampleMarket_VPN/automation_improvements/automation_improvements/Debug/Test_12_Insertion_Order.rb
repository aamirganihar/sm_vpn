...
describe "Test 12: Insertion Order page" do
  ...
  before(:all) do
    ...
    it "Test to check if Client Information section is displayed correctly from usamp Admin on the Insertion Order page" do
      sleep 3
      body_text = @browser.text
      body_text.should include("Client Information: Company: Automation client Contacts: Nitin Kumar Address: n/a Tax ID: n/a Phone: n/a Fax: n/a")
    end
    ...
    it "Test to check if Project Information section is displayed as per data from Basic info of Project on the Insertion Order page" do
      body_text = @browser.text
      body_text.should include("Project Details Project Name: #{@prj_name}")
      body_text.should include("Category: Business Survey Length: 10")
      body_text.should include("Estimated Start Date: #{@date_added_1} 01:00 AM PDT")
      body_text.should include("Estimated End Date: #{@date_added_10} 01:00 AM PDT Estimated Field Period: 9 days 0 hour 0 minute")
    ...
    it "Test to check if Group information section displays correct data for all groups and the reward amount on the Insertion Order page" do
      body_text = @browser.text
      body_text.should include("Completes Needed (N=): 25 Channel Completes Needed (N=) Reward Amount Estimated CPI Cost BuzzBack 5 $1.50 $1.00 $5.00 FocuslineSurveys 5 $1.50 $1.00 $5.00 Surveyhead 5 $1.50 $1.00 $5.00 [Your Panel Site] 5 $1.50 $1.00 $5.00 uSamp 5 included $5.85 *$29.25 **$49.25")
      body_text.should include("Estimated Costs for all groups **$49.25 * The costs described for uSamp panelists are an estimate based upon the information provided (Incidence (IR), length of interview (LOI) and any targeting used. Final cost will be based on the actual incidence (calculated as # of completes divided by # of completes + # of terminates/overquotas), and actual length of interview and may be higher/lower than the estimate described above. Any changes to targeting may also affect the final CPI as well. ** Final costs are calculated based on actual incidence and final completes.")
    ...
    it "Test to check if project terms section should display data from Self service Cost and terms in usamp admin on the Insertion Order page" do
      ...
      body_text.should include("The client agrees to the following fees and amounts for this project. CPI and Reward Costs: $49.25 (estimated) Email Costs: $10.00 / email Project Fee: $1.00")
