...
#    @browser.link(:text,/#{project_name['project'][1]}/).click
    ...
    @browser.select_list(:id,"optCountryId").select("Syria")
    grp_name= Time.now
    ...
    @browser.text.should include("uSamp $5.85")
  ...
  it "To check to see if client can set fielding limits,if usamp channel is selected for the group." do
    ...
    sleep 2
    @browser.text.should include("Set fielding limits for uSamp channel Complete as soon as possible Completes need to be spread evenly over the field period NOTE: Choosing the second option may affect feasibility and lengthen field time for hard to reach audiences.")
