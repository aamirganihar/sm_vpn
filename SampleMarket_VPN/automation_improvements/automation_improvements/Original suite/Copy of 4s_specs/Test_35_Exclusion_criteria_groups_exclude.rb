...
describe "Test 35: Exclusion criteria based on Projects" do
...
#    @browser.link(:text,/#{project_name['project'][1]}/).click
    it "Test to check if a client can successfully add a exclusion criteria for a member not to see a survey based on his previvious clicks/completes" do
        ...
        @browser.checkbox(:id,"chkMemberGroupsExclude").set
        sleep 2
        @browser.checkbox(:id,"chkExcludeClicks").set
        @browser.checkbox(:id,"chkExcludeCompletes").set
        @browser.checkbox(:id,"chkExcludeFails").set
        @browser.checkbox(:id,"chkExcludeOverQuota").set
        @browser.link(:id,"selExcludeQGhref").click
      ...
      puts @dec_group_id
      #@dec_group_id = "chkGroup[" + @dec_group_id +"]"
    ...
    it "Test to check if a is not shown a survey if he is set to be excluded on the basis of exclusion criteria" do
      sleep 2
  ...
  it "Test to check if a is not shown a survey if he is not set to be excluded on the basis of exclusion criteria" do
      sleep 2
      #Username = File.open("InputRepository/migrationdata.yml"){|file| YAML::load(file)}
    ...
    @browser.link(:text,"Log Out").click
    @browser.close
    puts "Test case 35 has completed"
    end
