...
class Test_30_External_prescreener < Test::Unit::TestCase
	  ...
	  def test_02_set_prescreener_admin
			...
			begin	
				...
				sleep 2
				$ie.radio(:xpath, "(//input[@name='rdPrescreenerOption'])[3]").click
				sleep 2
      #  $ie.radio(:id, "radPreSurveyActiveStatus").set
        #        sleep 2
				...
				if ($ie.text.include?('Pre-Survey redirect links'))
			...
			begin	
			   ...
			   $ie.goto("q.usampadmin.com/list_quota_group_publishers.php?project_id=#{$prj_n}&quota_group_id=#{$qg_n}")
                ...
                #$ie1=Watir::IE.attach(:title,'TEST_AUTOMATION_PRE_SURVEY')
				 #$ie.window(:title => "TEST_AUTOMATION_PRE_SURVEY").use do
				$ie.window(:title => "Welcome Back! Survey is closed").use do
                #$ie1.speed = :fast
                $ie.goto('http://q.u-samp.com/pre-redirect.php?S=1')
				sleep 5
				#$ie1.close
	  ...
	  def test_04_presurvey_fail
			...
			begin	
                ...
                #$ie1=Watir::IE.attach(:title,'TEST_AUTOMATION_PRE_SURVEY')
				 $ie.window(:title => "Welcome Back! Survey is closed").use do
                #$ie1.speed = :fast
                $ie.goto('http://q.u-samp.com/pre-redirect.php?S=2')
                sleep 10
				#$ie1.close
                #$ie=Watir::IE.attach(:title,'uSamp.com')
                #$ie.speed = :fast
