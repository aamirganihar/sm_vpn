...
#$file_1.close;
   ...
   # $file_2 = File.open($qg_id_file_path)
    ...
    sleep 2
    $ie.select_list(:id, "optCategoryId").select($match_cat)
  ...
  def test_03_publisher_category_filter_mismatch
   ...
   # $file_1 = File.open($proj_id_file_path)
    ...
    $ie.goto("http://q.usampadmin.com/add_publisher_settings.php?hdMode=settings&publisher_id=#{$pub_enc_id}")
    sleep 2
    $ie.select_list(:id, "optSelectedCategoryId").select($match_cat)
    $ie.button(:id,"btnRemove").click
    sleep 1
    $ie.select_list(:id, "optCategoryId").select($mismatch_cat)
    #$ie.button(:index,"3").click
    $ie.button(:id=>"btnAdd").click
    sleep 1
    $ie.button(:value,"Save").click
	...
	puts "FAIL"     
      ...
      $ff.goto("http://www.sm.q.surveyhead.com/index.php?mode=logout")
      $ff.close
      $ie.select_list(:id, "optSelectedCategoryId").select($mismatch_cat)
       $ie.button(:id,"btnRemove").click
      sleep 1
