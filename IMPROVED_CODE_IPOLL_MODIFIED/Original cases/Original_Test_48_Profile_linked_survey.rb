...
class Test_48_Profile_linked_survey < Test::Unit::TestCase
	  ...
	  def test_03_setting_profile_link
			...
			begin	
				...
				$ie.select_list(:id, "optProfileLinkId").set($prf_nm)
				$ie.button(:value,"Save Group").click
				
				if($ie.radio(:id, "rdSurveyURLType3").isSet?)
					puts 'Pass'
					$myfile.print "<td class=\"td3\"><font color=\"green\">TEST PASSED</font></td></tr>"
				else
					puts 'Fail'   
				...
				sleep 6
				$ie.select_list(:name, "optEthnicity[]").clear
	  ...
	  def test_05_revert_QG_settings
			...
			begin	
				...
				$ie.text_field(:id, "txtSurveyURL").set("http://www.s.instant.ly/s/XRtQG")
				$ie.button(:value,"Save Group").click
				if($ie.radio(:id, "rdSurveyURLType1").isSet?)
					puts 'Pass'
					$myfile.print "<td class=\"td3\"><font color=\"green\">TEST PASSED</font></td></tr>"
				else
					puts 'Fail'   
				...
				$ie.goto("p.usampadmin.com/add_quota_group_criteria.php?project_id=#{$prj_n}&quota_group_id=#{$qg_n}")
				sleep 6
				$ie.select_list(:name, "optEthnicity[]").set("Native American, Eskimo, Aleutian")
                $ie.select_list(:name, "optIncomeLevels[]").set("Prefer not to answer")
                $ie.select_list(:name, "optNationalityId[]").set("Mayotte")
                $ie.select_list(:name, "optEducationLevels[]").set("Advanced degree")
				$ie.button(:value,"Save Group").click 
				sleep 3
