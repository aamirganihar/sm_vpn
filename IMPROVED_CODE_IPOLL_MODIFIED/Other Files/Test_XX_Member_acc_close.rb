...
class Test_XX_Member_acc_close < Test::Unit::TestCase
	  ...
	  def test_02_memb_acc_close
			...
			begin	
				...
				if($ff.html.include?('Home'))
                    ...
                    if ($ff.text.include?('Your account has been successfully closed.'))
	  ...
	  def test_03_reopen_memb_acc
			...
			begin	
				...
				if($ie.html.include?('Member details have been updated successfully') && $ie.radio(:name, "rdMemberAccountClosed").set?)
					puts 'Pass'
					$myfile.print "<td class=\"td3\"><font color=\"green\">TEST PASSED</font></td></tr>"				
				else
					puts 'Fail'   
