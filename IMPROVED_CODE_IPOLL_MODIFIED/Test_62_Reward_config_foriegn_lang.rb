...
class Test_62_Reward_config_foriegn_lang < Test::Unit::TestCase
	  ...
	  def test_02_reward_config_spanish
			...
			begin	
				...
				sleep 4
				$ie.select_list(:name, "optLanguageId").select("Spanish")
				sleep 5
				...
				$ie.execute_script("CKEDITOR.instances.txtCatalogDesc.setData('<p>Nombre de recompensa : %%rewardname%% Nombre de pila : %%firstname%%</p>');")
				#$ie.frame(:title , 'Rich text editor, txtCatalogDesc, press ALT 0 for help.').send_keys 'Nombre de recompensa, Nombre de pila'
				sleep(2)
				#$ie.execute_script("CKEDITOR.instances['txtDescription'].setData('<p>Nombre de recompensa:%%rewardname%%Nombre de pila:%%firstname%%</p>');") 
				$ie.execute_script("CKEDITOR.instances.txtDescription.setData('<p>Nombre de recompensa:%%rewardname%%Nombre de pila:%%firstname%%</p>');")
				#$ie.frame(:title , 'Rich text editor, txtDescription, press ALT 0 for help.').send_keys 'Nombre de recompensa, Nombre de pila'
				sleep(1)
				...
				$ie.execute_script("CKEDITOR.instances.txtRedemption.setData('<p>regalo codigo:%%giftcode%%Cantidad de regalos:%%giftamount%%regalo Url:%%gifturl%%</p>');")
				sleep(1)
				#$ie.frame(:title , 'Rich text editor, txtRedemption, press ALT 0 for help.').send_keys 'Nombre de recompensa, Nombre de pila'
				$ie.text_field(:id, "txtSubject").set "Solicitud de donacion espanola"
				$ie.execute_script("CKEDITOR.instances.txtEmailDesc.setData('<p>Querido %%firstname%%</p>');")
				sleep(1)
				#$ie.frame(:title , 'Rich text editor, txtEmailDesc, press ALT 0 for help.').send_keys 'Querido'
				$ie.button(:value,"Update").click
				$st = '1'
				$test_description = "Configuring reward in foreign language (Spanish)"
				$myfile.print "<tr><td class=\"td1\">"+$st+"</td>"
				$myfile.print "<td class=\"td2\">"+$test_description+"</td>"
				if($ie.html.include?('Reward Type has been updated successfully'))
					puts "passed"
	  ...
	  def test_03_check_rew_desc
			...
			begin	
				...
				sleep 5
				$ff.goto("http://www.q.ipoll.com/reward_catalog.php?type=donation")
				sleep 5
				...
				$myfile.print "<tr><td class=\"td1\">"+$st+"</td>"
				$myfile.print "<td class=\"td2\">"+$test_description+"</td>"
				if($ff.text.include?($don_spn) && $ff.html.include?('Nombre de recompensa'))  
					puts "desc_pl_passed"
					$myfile.print "<td class=\"td3\"><font color=\"green\">TEST PASSED</font></td></tr>"
				else
					puts "desc_pl_failed"
