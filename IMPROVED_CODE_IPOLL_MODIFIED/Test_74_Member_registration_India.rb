...
require './Input Repository\Test_74_Member_registration_India_Input.rb'
gem "selenium-client"
...
class Test_01_Project_QG_Create < Test::Unit::TestCase
      ...
      def test_01_report_head
				...
				$t = "74 - "
				$test_description = "Test Case: "+$t.to_s+"  INDIA MEMBER REGISTRATION (SURVEYHEAD/FOCUSLINE)"
				$obj = Usamp_lib.new
	  ...
	  def test_02_member_signup_SH
			...
			begin
                ...
                $ff = Watir::Browser.new driver
                  ...
                  sleep 2
                  $ff.select_list(:name, "optCityId").select($city)
                  sleep 2
		...
		def test_05_member_signup_Focusline
				...
				begin
                    ...
                    $ff = Watir::Browser.new driver
					  ...
					  $ff.goto("http://sm.p.surveyhead.com/registration_step1.php?P=#{$pub_id}&MID=#{$mid}&S=#{$s}&SP=#{$sp}")
					  $ff.text_field(:name, "txtFname").set("AUTO FNAME")
					  $ff.text_field(:name, "txtLname").set("AUTO LNAME")
					  $ff.select_list(:name, "optCountryId").select($country)
					  $ff.select_list(:name, "optLanguageId").select($lang)
					  ...
					  $ff.select_list(:name, "optStateId").select($state)
					  sleep 2
            $ff.select_list(:name, "optCityId").set($city)
            sleep 2
            $ff.text_field(:name, "txtZipPostal").set($zip_match)
		...
		def test_06_member_in_admin_Focusline
		  ...
		  def test_07_mail_Focusline
			...
			begin
				...
				sleep 3
        #puts $ff1.text
				if ($ff1.html.include?("Your FocuslineSurveys Account."))
					$myfile.print "<td class=\"td3\"><font color=\"green\">TEST PASSED</font></td></tr>"	
				else
					$myfile.print "<td class=\"td3\"><font color=\"red\">TEST FAILED</font></td></tr>"	
				end
