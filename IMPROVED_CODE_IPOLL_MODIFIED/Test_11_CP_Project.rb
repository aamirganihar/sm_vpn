...
class Test_01_Project_QG_Create < Test::Unit::TestCase
	  ...
	  def test_02_create_cp_project
			...
			begin
					...
					if ($ff.text.include?('Suppliers') && $ff.text.include?(cpp_name) )
	  ...
	  def test_04_cp_survey_complete
		   ...
		   driver = Selenium::WebDriver.for :ie #, #:profile => "Selenium"
			...
			begin
				...
				#$ie2 = Watir::IE.new
                 ...
                 #$ie2.speed = :fast
                  ...
                  $ie2.goto($survey_url)
                  sleep 8
                  $ie2.goto("http://q.mr1mr.com/redirect.php?S=2&PRID=#{$enc_shield_id}&ID=")
                  sleep 4
                 $ff.link(:text,'Stats').click
                  sleep 2
                 $html_contents=$ff.html
			...
			begin
					  ...
					  #$ie3 = Watir::IE.new
					   driver = Selenium::WebDriver.for :ie #, #:profile => "Selenium"
				            $ie3 = Watir::Browser.new driver
					  #$ie3.speed = :fast
					  $ie3.goto($survey_url)
					  sleep 10
					  
					  $ie3.goto("http://q.mr1mr.com/redirect.php?S=3&PRID=#{$enc_shield_id}&ID=")
					  sleep 4
					  $ff.link(:text,'Stats').click
					  sleep 2
					  $html_contents=$ff.html
	  ...
	  def test_05_cp_project_admin
			...
			begin
					...
					$ie.goto("http://q.usampadmin.com/ps_projects_listing.php")
					sleep 2
					$ie.select_list(:id, "clientId").select($client)
                    $ie.button(:value,"Search").click
					$st = '6'
					$test_description = "Searching Client panelshield project in admin"
					$myfile.print "<tr><td class=\"td1\">"+$st+"</td>"
					$myfile.print "<td class=\"td2\">"+$test_description+"</td>"
					if($ie.html.include?('Test_CP_PROJ_'))
							puts "Search Passed"
							$myfile.print "<td class=\"td3\"><font color=\"green\">TEST PASSED</font></td></tr>"         
							sleep 5
							...
							if ($ie.select_list(:id,"surveyLength").value == "6")
								  ...
								  $ie.select_list(:id, "surveyLength").select($survey_length)
								   sleep 5
								   $ie.button(:value,"Save Project").click
								   sleep 5
							...
							sleep 5
							$ie.select_list(:name, "supplierList").select($sup_name_2)
	  ...
	  def test_06_project_close
			...
			begin
					  ...
					  if($ie.html.include?('Test_CP_PROJ_'))
							...
							sleep 5
							  $ie.link(:text, "Project Details").click
							  $ie.select_list(:id, "projectStatus").select("CLOSED")
							  sleep 3
							  $ie.button(:value,"Save Project").click
