...
class Test_31_Merge_field_survey_url < Test::Unit::TestCase
	  ...
	  def test_02_update_QG
		...
		begin	
			...
			sleep 2
			$ie.text_field(:id, "txtSurveyURL").set("http://203.199.26.75/usamp/TEST_SURVEY.php?SHA=%%sha%%&MEM_NO=%%memberno%%&ENC_MEM_NO=%%encryptedmemberno%%&AGE=%%age%%&CHILDRN=%%children%%&ETHNICITY=%%ethnicity%%&EDUC=%%education%%&GEN=%%gender%%&INCOME=%%income%%&MAR_STATUS=%%marital%%&ZIP=%%zip%%&PID=%%pid%%&SP_ID=%%sp%%&EMAIL=%%emailaddress%%&COUNTY=%%countyfips%%&STATE=%%statefips%%&DMA=%%dma%%&TOKEN=%%token%%")
			sleep 2
	  ...
	  def test_03_check_merge_fields
		...
		begin	
			...
			$ff.window(:title, "TEST_AUTOMATION_SURVEY").use do
			puts "New URL"
			$url= $ff.url
			puts $url
			length = $url.length
			...
			$myfile.print "<tr><td class=\"td1\">"+$st+"</td>"
			$myfile.print "<td class=\"td2\">"+$test_description+"</td>"
			if ($url == "http://203.199.26.75/usamp/TEST_SURVEY.php?SHA=218dc9724725dfc0f1ba4d3da606ee266d3ade60&MEM_NO=10598227&ENC_MEM_NO=Y4vwDOmmu&AGE=38&CHILDRN=2&ETHNICITY=9&EDUC=6&GEN=1&INCOME=22&MAR_STATUS=1&ZIP=90001&PID=0&SP_ID=&EMAIL=gsk@hk.com&COUNTY=06037&STATE=06&DMA=803&TOKEN=") 
				puts "pass"
				$myfile.print "<td class=\"td3\"><font color=\"green\">TEST PASSED</font></td></tr>"
			else
				$myfile.print "<td class=\"td3\"><font color=\"red\">TEST FAILED</font></td></tr>" 
