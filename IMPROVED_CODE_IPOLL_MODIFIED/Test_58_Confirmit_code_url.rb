...
class Test_58_Confirmit_code_url < Test::Unit::TestCase
	  ...
	  def test_04_conf_code_qg_auto_close
			...
			begin	
                ...
                $ff.button(:value,"START SURVEY").click
                sleep 10
				 $ff.window(:title,"Welcome Back! Survey is closed").use do
				...
				$ie.goto("q.usampadmin.com/add_quota_group.php?project_id=#{$prj_n}&quota_group_id=#{$qg_n}")
				sleep 1
				$ie.goto("http://q.usampadmin.com/quota_group_statistics.php?project_id=#{$prj_n}&quota_group_id=#{$qg_n}&static=")
				sleep 5
				$st = '5'
