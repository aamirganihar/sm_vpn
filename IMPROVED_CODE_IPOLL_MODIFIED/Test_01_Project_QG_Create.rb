...
class Test_01_Project_QG_Create < Test::Unit::TestCase
  ...
  def test_01_report_head
    $t = "1 - "
    $test_description = "Test Case: "+$t.to_s+"  PROJECT/QUOTA GROUP CREATION"
    $obj = Usamp_lib.new
    ...
    $st = "1"
    $test_description = "Project Creation"
...
=begin
    timeout = 45
    start_time = Time.now
    if($ie.div(:name=>"txtSampleEndDate").exists?)
      break
      else
      while $ie.div(:id=>"txtSampleEndDate").exists? do
      sleep 0.5
...
=end
    ...
    sleep 5
    if ($ie.html.include?(prj_name) && $ie.html.include?('Project details have been added successfully'))
    ...
    #$ie.button(:xpath, ".//*[@id='datepicker']/table/tbody/tr[1]/td[3]/button").click
    #sleep 5
    #$ie.td(:xpath, ".//*[@id='datepicker']/table/tbody/tr[4]/td[4]").click
    qg_name= Time.now
    ...
    sleep 6
    if ($ie.html.include?(qg_name) && $ie.html.include?('Criteria'))
  ...
  def test_04_Save_criteria
    ...
    sleep 5
    if ($ie.text.include?('Quota group criteria details have been saved successfully'))
      $gc = 1
    end   
   ...
   if ($ie.html.include?('Quota group Member Publisher details have been updated successfully'))
    ...
    sleep 5
    if ($ie.text.include?('Panelshield details have been updated successfully'))
      $psc = 1
    end
