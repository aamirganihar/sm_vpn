...
class Test_57_Success_fail_oq_rewards < Test::Unit::TestCase
	  ...
	  def test_02_setting_fail_oq_rewards
			...
			begin	
				...
				if($ie.checkbox(:id, "chkFailRewardStatus").set?)
					puts "PASS"
				...
				$ie.button(:value,"Done").click
				sleep 10
				if($ie.checkbox(:id, "chkOverQuotaRewardStatus").set?)
					puts "PASS"
					$myfile.print "<td class=\"td3\"><font color=\"green\">TEST PASSED</font></td></tr>"
				else
					puts "FAIL"
				...
				$ie.link(:xpath, ".//*[@id='trQgRewardInfo']/td[2]/a").click
				#$ie.link(:text => "Edit / view History").click
				sleep 5
				$ie.button(:value,"Add New").click
				sleep 5
				$ie.radio(:name, "rdRewardType").click
				...
				$myfile.print "<tr><td class=\"td1\">"+$st+"</td>"
				$myfile.print "<td class=\"td2\">"+$test_description+"</td>"
				if($ie.html.include?('New Reward value has been added successfully'))
					puts "PASS"
					$myfile.print "<td class=\"td3\"><font color=\"green\">TEST PASSED</font></td></tr>"
				else
					puts "FAIL"
			...
			begin		
		     ...
		     $ff.window(:url,"https://www.q.ipoll.com/dashboard.php").use do
              ...
              puts $html_array[index+13]
							puts "********"
							if (($html_array[index+12] =~ /Earned/) && ($html_array[index+6] =~ /\$5.00/))
                                    puts "PASS"
                                    $myfile.print "<td class=\"td3\"><font color=\"green\">TEST PASSED</font></td></tr>"
                            else
                                    puts "FAIL"
