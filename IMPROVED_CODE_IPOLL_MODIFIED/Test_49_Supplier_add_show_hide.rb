...
class Test_49_Supplier_add_show_hide < Test::Unit::TestCase
	  ...
	  def test_02_add_supplier
			...
			begin	
				...
				sleep 3
				$ie.goto("http://q.usampadmin.com/ps_add_supplier.php")
				...
				if($ie.html.include?($supplier_name))
					puts 'Pass'
	  ...
	  def test_03_supplier_reflection
			...
			begin	
				...
				sleep 6
				$ff.goto("https://q.network.u-samp.com/pshield/ps_welcome.php")
				$ff.goto("https://q.network.u-samp.com/pshield/ps_projects_listing.php?type=open")
				sleep 3
				$enc_sheild_id = Base64.encode64($cp_sheild_id)
				$ff.goto("https://q.network.u-samp.com/pshield/ps_project_suppliers.php?hdPsPrjId=#{$enc_sheild_id}")
				sleep 5
				$flg = 0
	  ...
	  def test_04_show_hide
			...
			begin	
				...
				$ie = $obj.Usampadmin_login($admin_email,$admin_passwd)
				sleep 3
				$ie.goto("http://q.usampadmin.com/ps_suppliers_listing.php")
				sleep 3
				$ie.link(:text,"Supplier ID").click
				sleep 3
				...
				#$sup_ary = $ff.select_list(:id,"supplierList").getAllContents
				elem = Array.new
				elem = $ie.select_list(:id, "supplierList").options
				#$ln = $sup_ary.length
				$ln = elem.length
				0.upto($ln - 1) { |index|
				if(elem[index] == $supplier_name)
