...
class Test_66_No_of_QG_clicks_allowed < Test::Unit::TestCase
	  ...
	  def test_02_set_no_of_clicks_QG
			...
			begin	
				...
				if ($ie.checkbox(:id, "chkQuotaMultipleClicks").set? && $val == $no_of_dashboard_clicks_allowed)
					puts "PASS"
	  ...
	  def test_04_change_no_of_clicks_setting
			...
			begin	
				...
				$myfile.print "<td class=\"td2\">"+$test_description+"</td>"
				$val = $ie.text_field(:id, "txtQuotaMaxClicks").value
				if ($ie.checkbox(:id, "chkQuotaMultipleClicks").set? && $val == $no_of_dashboard_clicks_allowed_2)
					puts "PASS"
					$myfile.print "<td class=\"td3\"><font color=\"green\">TEST PASSED</font></td></tr>"
				else
					puts "FAIL"
	  ...
	  def test_07_revert_settings
			...
			begin	
				...
				$myfile.print "<tr><td class=\"td1\">"+$st+"</td>"
				$myfile.print "<td class=\"td2\">"+$test_description+"</td>"
				if ($ie.checkbox(:id, "chkQuotaMultipleClicks").set?)
					puts "FAIL"
					$myfile.print "<td class=\"td3\"><font color=\"red\">TEST FAILED</font></td></tr>"
				else
					puts "PASS"
