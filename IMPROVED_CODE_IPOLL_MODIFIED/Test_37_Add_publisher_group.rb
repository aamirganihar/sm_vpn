...
class Test_37_Add_publisher_group < Test::Unit::TestCase
	  ...
	  def test_02_add_pub_group
			...
			begin	
				...
				sleep 2
				$ie.goto("http://q.usampadmin.com/add_publisher_groups.php")
				$ext = Time.new
				$ext = $ext.to_s
				...
				$ie.select_list(:name,'optPublisherId').select($pub_company.chomp)
				$ie.button(:name,'btnAdd').click
				$ie.button(:name,"SUBMIT").click
				if ($ie.html.include?('Publisher group has been created successfully'))
					puts 'Pass'
	  ...
	  def test_03_search_edit_pub_group
			...
			begin	
				...
				$ie = $obj.Usampadmin_login($admin_email,$admin_passwd)
				sleep 2
				$ie.goto("http://q.usampadmin.com/list_publisher_groups.php")
				$file_1 = File.open($pub_grp_name_file_path)
				$pub_grp = $file_1.gets
				puts $pub_grp
				$file_1.close;
				
				$ie.select_list(:name,'optPublisherGroupsOptionId').select($pub_grp.chomp)
				...
				$myfile.print "<tr><td class=\"td1\">"+$st+"</td>"
				$myfile.print "<td class=\"td2\">"+$test_description+"</td>"
				$gn = "TEST TEMP GRP NAME"+$ext_1
				puts $gn
				$ie.text_field(:name,'txtGroupName').set($gn)
				sleep 5
				$ie.select_list(:name,'optPublisherId').select($test_pub)
				sleep 3
				$ie.button(:name,'btnAdd').click
				$ie.button(:name,"SUBMIT").click
				sleep 4
				...
				if($ie.html.include?('updated successfully') && $grp == $gn)
					puts 'Pass'
					$myfile.print "<td class=\"td3\"><font color=\"green\">TEST PASSED</font></td></tr>"
				else
					puts 'Fail'   
				...
				sleep 3
				$ie.select_list(:name,'optSelectedPublisherId').select($test_pub)
