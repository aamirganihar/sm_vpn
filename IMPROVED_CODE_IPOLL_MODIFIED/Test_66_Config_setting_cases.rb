...
class Test_67_Config_setting_cases < Test::Unit::TestCase
	  ...
	  def test_02_saving_config_setting
			...
			begin	
				...
				if ($ie.html.include?('Global values have been updated successfully') && ($val1 == $rew_cashout)) #&& ($val3 == $cap_amt)) #&& ($val2 == $fcpi) 
					puts "PASS"
	  ...
	  def test_04_check_reward_cost
			...
			begin	
				...
				$ie.goto("q.usampadmin.com/add_quota_group.php?project_id=#{$prj_n}&quota_group_id=#{$qg_n}")	
				sleep 2
				$ie.goto("q.usampadmin.com/quota_group_statistics.php?project_id=#{$prj_n}&quota_group_id=#{$qg_n}")
				sleep 3
				$st = '3'
