...
#    @browser.link(:text,/#{project_name['project'][1]}/).click
    ...
    sleep 2
    @browser.checkbox(:id,"chkAutoCloseClicks").set(false)
...
=begin
   it "To click on a survey link 1" do
     ...
     @browser1.window(:title => /Survey/).use do
	     @url=@browser1.url
	     #@browser1.goto("http://www.s.instant.ly/s/XRtQG")
   ...
   it "To click on a survey link 3" do
      sleep 10
     group_id = File.open("InputRepository/Group_Ids_limits.yml"){|file| YAML::load(file)}
     @dec_group_id =  group_id['group_id']
     group_id = File.open("InputRepository/Group_Ids_limits.yml"){|file| YAML::load(file)}
   ...
   it "To click on a survey link 4" do
      sleep 10
     group_id = File.open("InputRepository/Group_Ids_limits.yml"){|file| YAML::load(file)}
     @dec_group_id =  group_id['group_id']
     group_id = File.open("InputRepository/Group_Ids_limits.yml"){|file| YAML::load(file)}
   ...
   it "To click on a survey link 5" do
      sleep 10
     group_id = File.open("InputRepository/Group_Ids_limits.yml"){|file| YAML::load(file)}
     @dec_group_id =  group_id['group_id']
     group_id = File.open("InputRepository/Group_Ids_limits.yml"){|file| YAML::load(file)}
...
#  end
   ...
   it "To click on a survey links" do
	   sleep 10
	   group_id = File.open("InputRepository/Group_Ids_limits.yml"){|file| YAML::load(file)}
	   @dec_group_id =  group_id['group_id']
	   group_id = File.open("InputRepository/Group_Ids_limits.yml"){|file| YAML::load(file)}
	  ...
	  # puts"survey1"
	   ...
	   sleep 10
	   @browser1.window(:title => /Survey/).use do
		  @browser1.goto(BasicData::SURVEY_URL)
	   end
	   @browser1.close
	   sleep 1
...
#     end
     #puts"survey2"
     @browser2 = @obj.Surveyhead_sm_login("test22.des02@mailop.com","test22.des02@mailop.com")
     
     @browser2.link(:id,"#{@dec_group_link}").click
     sleep 2
     @browser2.button(:name,'Submit').click
     sleep 2
    @browser2.window(:title => /Survey/).use do
	   @browser2.goto(BasicData::SURVEY_URL)
    end
     @browser2.close
     sleep 1
     #if (@browser1.div(:id=>"loadingSurveys").exists?)
  ...
  #   end
       #puts"survey3"
    @browser3 = @obj.Surveyhead_sm_login("test22.des02@mailop.com","test22.des02@mailop.com") 
     sleep 4
     @browser3.link(:id,"#{@dec_group_link}").click
     sleep 2
     @browser3.button(:name,'Submit').click
     sleep 2
@browser3.window(:title => /Survey/).use do
     @browser3.goto(BasicData::SURVEY_URL)
   ...
   #  if (@browser1.div(:id=>"loadingSurveys").exists?)
	...
	#	     puts "waiting for surveys to load"
     ...
     #end
          #puts"survey4"
     @browser4 = @obj.Surveyhead_sm_login("test22.des02@mailop.com","test22.des02@mailop.com")
     sleep 4
     @browser4.link(:id,"#{@dec_group_link}").click
     sleep 2
     @browser4.button(:name,'Submit').click
     sleep 2
     @browser4.window(:title => /Survey/).use do
     @browser4.goto(BasicData::SURVEY_URL)
	...
	#	     puts "waiting for surveys to load"
     ...
     #end
          #puts"survey5"
     @browser5 = @obj.Surveyhead_sm_login("test22.des02@mailop.com","test22.des02@mailop.com")
     sleep 4
     @browser5.link(:id,"#{@dec_group_link}").click
     sleep 2
     @browser5.button(:name,'Submit').click
     sleep 2
    @browser5.window(:title => /Survey/).use do
     @browser5.goto(BasicData::SURVEY_URL)
  ...
  it "To check if the USAMP Channel changes to pauses state after the waiting period" do
    ...
    #@browser.text.should include("uSamp channel is currently paused for an incidence check and will resume shortly")
