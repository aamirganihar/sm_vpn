...
describe "Test 34: Test to check the functionality of Exclude PL Site Members from SampleMarket" do
  ...
  before(:all) do
    ...
    it "To obtain Review Counts with Demographic Criteria for the feasibility report with the Exclude PL Site Members from SampleMarket checkbox not set" do
       ...
       #driver = Selenium::WebDriver.for :firefox, :profile => "Selenium"
        ...
        @browser2.goto("https://p.network.u-samp.com/autologin.php?empid=257&email=nitin_kumar@persistent.co.in&client_id=692&appType=admin")
      ...
      #sleep 3
        ...
        Watir::Wait.until { @browser2.text.include? 'Age' }#explicit wait:default timeout :30sec:HRISHI
      @browser2.link(:text,/Age/).click
      Watir::Wait.until { @browser2.text.include? 'Age Range' }#explicit wait:default timeout :30sec:HRISHI
        #while @browser2.div(:id=>"fancybox-loading").visible? do
          #sleep 0.5
          #puts "waiting for element"
  #end
  #sleep 5
	...
	#@browser.div(:id =>"roundBlue curvyRedraw").radio(:id => "priceYes").set
	@browser2.radio(:id,"priceYes").set
	sleep 2
	@browser2.text_field(:id,"txtSampleSize").set("20")
	@browser2.text_field(:id,"idSurveyLength").set("10") 
	@browser2.text_field(:id,"idIncidenceEst").set ("50")
