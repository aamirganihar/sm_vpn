...
describe "Test 21: Report generation" do
  ...
  before(:all) do
    ...
    it "To Check if user is able to to run the Panel SignUp report and download it (All Time)" do
      ...
      sleep 5
	  basedir = "C:\\Documents and Settings\\Administrator\\My Documents\\Downloads\\"
	  #basedir =  "C:\\Users\\tanvi_deodhar\\Downloads\\"
	  downloads_before = Dir.new(basedir).entries
      @browser.link(:text,"Download these results").click
	  
      sleep 20
      #contains = Dir.new(basedir).entries
	  30.times do
	  difference = Dir.new(basedir).entries - downloads_before
	  if difference.size == 1
			@file = difference.first 
			break
	  end 
	  sleep 1
	  end
