...
=end    
	    ...
	    #end
	    #sleep 2
	    Watir::Wait.until { @browser.text.include? 'Supplier Redirect Links' }#explicit wait:default timeout :30sec
    ...
    it "Test to check if we can add an existing supplier" do
	    ...
	    sleep 4
	    @browser.select_list(:id,"optExistingSuppliersId").select("testsup")
