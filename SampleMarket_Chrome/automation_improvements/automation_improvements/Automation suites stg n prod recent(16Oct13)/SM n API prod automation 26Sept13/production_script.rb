...
describe "Test 00: Production basic tests" do
  ...
  before(:all) do
	...
	@browser.text_field(:id,"txtSubject").set("Subject: Test automation broadcast3")
...
=end	
	...
	sleep 4
        ...
        body_text.should include("Subject: Subject: Test automation broadcast3")
...
=end      
	...
	sleep 5
		...
		#if (@browser1.text.include?("We are committed to improving your panel experience!")
		#	@browser1.button(:name,'Submit').click
		#else
		#	sleep 3
		#end
...
=begin
	...
	sleep 5
        ...
        if(@text_array[index] =~ /#{@dec_group_id}/)
            @text_array[index+4].should include("0.50") 
	    puts @text_array[index+1]
	    #@text_array[index+4].should include("0.50") or ("1.00")
	    #puts @text_array[index+1]
	    #puts @text_array[index+2]
	    #puts @text_array[index+3]
	    #puts @text_array[index+4]
...
=end    
      ...
      sleep 2
	    ...
	    body_text = @browser1.text
			...
			if (@browser1.text.include?("Congratulations") || @browser1.text.include?("SORRY")|| @browser1.text.include?("committed"))
				puts "Fail success"
			else
				puts "Fail  fail"
				break
...
=end      
    ...
    it "Test to check if the email broadcast is successfully received and a member is able to complete it" do
	    ...
	    sleep 15
	    #body_text = @browser1.text
	    @browser1.text.should include("subject: test automation broadcast3")	
		@browser1.link(:text,"subject: test automation broadcast3").click
